...
class Interpolater(object):
    ...
    """
    def __init__(self, interpolate_level=3, mesh = None):
    ...
    def _get_spoke_dirs(self, srep, num_crest_points=24):
        ...
        num_smooth_skeletal_pts = num_crest_points*3*2
        start, end = 0, num_smooth_skeletal_pts
        total_num_pts = srep.GetNumberOfPoints()
        if not self.interpolate_up:
            start, end = (start + num_smooth_skeletal_pts), (end + num_smooth_skeletal_pts)
        for i in range(start, end, 2):
            spoke = np.array(srep.GetPoint(i+1)) - np.array(srep.GetPoint(i))
    ...
    def interpolate_crest(self, input_srep, top_spokes, bot_spokes, num_crest_points):
        ...
        crest_appender = vtk.vtkAppendPolyData()
        for i in range(num_interp_crest_pts):
            ...
            for j in range(len(top_interp_crest_pts)):
                # if j ==0 or (j + 1) % num_steps == 0: continue
                ...
                crest_appender.AddInputData(top_spoke.visualize())
                crest_appender.AddInputData(bot_spoke.visualize())
        ...
        # p.show()
        crest_appender.Update()
        return crest_appender.GetOutput(), top_crest_spokes + bot_crest_spokes
    def interpolate_all(self, top_and_bot_and_crest, num_crest_points, num_samples_outward):
        interp_spokes, up_spokes = self.interpolate(top_and_bot_and_crest, num_crest_points, num_samples_outward)

        ## Interpolate down spokes
        self.interpolate_up = False
        interp_down_spokes, bot_spokes = self.interpolate(top_and_bot_and_crest, num_crest_points, num_samples_outward)

        ## interpolate fold spokes
        crest_poly, crest_spokes = self.interpolate_crest(top_and_bot_and_crest, up_spokes, bot_spokes, num_crest_points)
        
        # p = pv.Plotter()
        # p.add_mesh(self.mesh, color='white', opacity=0.3)
        # p.add_mesh(interp_spokes, color='orange', line_width = 4)
        # p.add_mesh(interp_down_spokes, color='red', line_width=4)
        # p.add_mesh(crest_poly, color='cyan', line_width=4)
        # p.show()

        def form_list_spokes(vtk_spokes):
            ret_list = []
            for i in range(0, vtk_spokes.GetNumberOfPoints(), 2):
                base_pt = vtk_spokes.GetPoint(i)
                bdry_pt = vtk_spokes.GetPoint(i+1)
                ret_list.append(Spoke(base_pt=base_pt, bdry_pt=bdry_pt))
            return ret_list
        top_spokes, bot_spokes = form_list_spokes(interp_spokes), form_list_spokes(interp_down_spokes)
        return top_spokes + bot_spokes + crest_spokes
    ...
    def interpolate(self, input_srep, num_crest_points, num_samples_outward):
        ...
        # p = pv.Plotter()
        # p.add_mesh(input_srep, color='gray', line_width=3)
        for r in range(num_crest_points):
            for c in range(num_samples_outward -1):
                ...
                for ri in range(num_steps + 1):
                    for ci in range(num_steps + 1):
                        ...
                        #     p.add_mesh(pv.Arrow(interp_skeletal_pt, interpolated_dir, scale=3), color='red', label='tau_1')
                        # elif ci in [0, num_steps] and c == 1:
                        #     p.add_mesh(pv.Arrow(interp_skeletal_pt, interpolated_dir, scale=3), color='cornflowerblue', label='theta')
        ...
        # p.add_legend(labels=[['', 'red'], ['', 'gray']], bcolor=(0.1, 0.1, 0.1))
        # p.show()
